<?php
    class Charge_model extends CI_Model {
        ...
        function __construct()
        {
			...
			$this->_charge_tb = $this->config->item('TABLE_CHARGE');
        }

        public function get_info($charge_id = NULL, $user_id = NULL)
        {
			$where = 'WHERE 1=1 ';
            if (!is_null($charge_id))
            {
                $where .= ' AND id = '.intval($charge_id);
            }
            ...
            $sql = "SELECT * FROM ".$this->_charge_tb." $where order by charge_date desc";
            
            return is_null($charge_id) ? $this->db_model->select($sql) : $this->db_model->select_one($sql);
        }
