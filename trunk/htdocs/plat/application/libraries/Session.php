...
class CI_Session {
	...
	function __construct()
	{
		...
		$this->_lifetime = $this->CI->config->item('sess_expiration');
		$this->_sess_id_ttl = $this->CI->config->item('sess_time_to_update');
		$this->_match_ip = $this->CI->config->item('sess_match_ip');
		$this->_match_useragent = $this->CI->config->item('sess_match_useragent');
		$this->_useDB = $this->CI->config->item('sess_use_database');
		$this->_sess_table = $this->CI->config->item('sess_table_name');
	...
	*/
	function _ips_match() {
		...
		if (!$this->userdata('sess_ip_address')) {
			$this->set_userdata('sess_ip_address',  $this->CI->input->ip_address());
			return TRUE;
		}

		return $this->userdata('sess_ip_address') == $this->CI->input->ip_address();
	}
	...
	*/
	...
	// if this is the first time coming in, initialize user agent
		if (!$this->userdata('sess_useragent')) {
			$this->set_userdata('sess_useragent', trim(substr($this->CI->input->user_agent(), 0, 50)));
			return TRUE;
		}

		return $this->userdata('sess_useragent') == trim(substr($this->CI->input->user_agent(), 0, 50));
	}
	...
	*/
	...
	function _open()
	{
		if ($this->_sess_db = mysql_connect($this->CI->db->hostname,
					$this->CI->db->username,
					$this->CI->db->password)) {
			return mysql_select_db($this->CI->db->database, $this->_sess_db);
		}
